 ...
 * @returns {any}
 */
export function listUpdatedFields(fileBytes) {
 ...
 * @returns {any}
 */
export function parseTicks(file, wanted_props, wanted_ticks, wanted_players, struct_of_arrays) {
    const ptr0 = passArray8ToWasm0(file, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
 ...
 * @returns {any}
 */
export function parseGrenades(file, extra, grenades) {
    const ptr0 = passArray8ToWasm0(file, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('demoparser2_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
